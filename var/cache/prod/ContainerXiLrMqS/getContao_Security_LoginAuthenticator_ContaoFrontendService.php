...
class getContao_Security_LoginAuthenticator_ContaoFrontendService extends Contao_ManagerBundle_HttpKernel_ContaoKernelProdContainer
{
    ...
    public static function do($container, $lazyLoad = true)
    {
        ...
        $j = new \Scheb\TwoFactorBundle\Security\TwoFactor\TwoFactorFirewallConfig(['remember_me' => true, 'require_previous_session' => false, 'auth_code_parameter_name' => 'verify'], 'contao_frontend', $f, ($container->privates['scheb_two_factor.security.request_data_reader'] ??= new \Scheb\TwoFactorBundle\Security\Http\Utils\RequestDataReader()));

        return $container->privates['contao.security.login_authenticator.contao_frontend'] = new \Contao\CoreBundle\Security\Authenticator\ContaoLoginAuthenticator(($container->privates['contao.security.frontend_user_provider'] ?? $container->load('getContao_Security_FrontendUserProviderService')), $a, $b, ($container->services['contao.routing.scope_matcher'] ?? self::getContao_Routing_ScopeMatcherService($container)), ($container->privates['router.default'] ?? self::getRouter_DefaultService($container)), ($container->services['uri_signer'] ??= new \Symfony\Component\HttpFoundation\UriSigner($container->getEnv('APP_SECRET'))), $c, $i, $d, $e, ($container->services['request_stack'] ??= new \Symfony\Component\HttpFoundation\RequestStack()), new \Scheb\TwoFactorBundle\Security\Http\Authenticator\TwoFactorAuthenticator($j, $i, $a, $b, new \Scheb\TwoFactorBundle\Security\Http\Authentication\DefaultAuthenticationRequiredHandler($f, $j), $g, $h), ['require_previous_session' => false, 'auth_code_parameter_name' => 'verify']);
    }
}
